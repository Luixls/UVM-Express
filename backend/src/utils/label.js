 ...
 * Si el usuario no tiene dirección, los campos de remitente se dejan en blanco.
 */
export const generateLabel = async (params) => {
  const tracking = String(params.tracking || '').trim();

  const fromName = params.fromName ?? null;
  const fromAddress = params.fromAddress || null; // puede ser cadena vacía
  ...
  try {
    const barcodePng = await bwipjs.toBuffer({ bcid:'code128', text:tracking, height:12, scale:3, includetext:false });
  ...
  } catch { doc.moveDown(2); }

  // Remitente (en blanco si no hay datos)
  ...
  if (!fromName && !fromAddress && !originCenter) doc.text(' '); // línea en blanco
  doc.moveDown(0.4);
  ...
  if (!toName && !toAddress && !toCityStateCountry) doc.text(' ');
  doc.moveDown(0.4);
  ...
  if (cajaInfo) { doc.moveDown(0.2); doc.fontSize(10).text(cajaInfo, { align: 'right' }); }
  ...
  doc.moveDown(0.6);
  doc.fontSize(7).fillColor('#444').text('Manipular con cuidado. Sujeto a términos de transporte UVM Express.', { align: 'center' });
  doc.fillColor('black');
