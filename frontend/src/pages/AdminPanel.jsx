...
const STATUS_OPTIONS = Object.entries(STATUS).map(([k, v]) => ({
  value: k,
  label: v,
}));
const input =
  "w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-50 px-3 py-2 outline-none placeholder:text-neutral-400 focus:ring-2 focus:ring-green-600";
...
export default function AdminPanel() {
  ...
  const fetchPackages = async () => {
    try {
      ...
      const { data } = await api.get("/admin/packages", {
        params: { q: pkQ, status: pkStatus, page: 1, limit: 50 },
      });
      if (!data?.ok) throw new Error(data?.error || "Error");
  ...
  const loadShipmentPackages = async () => {
    if (!shipmentTracking.trim())
      return toast.error("Indica tracking de envío o de paquete");
    try {
      setShipLoading(true);
      const { data } = await api.get(
        `/admin/shipments/${encodeURIComponent(
          shipmentTracking.trim()
        )}/packages`
      );
      if (!data?.ok)
        throw new Error(data?.error || "No se pudo obtener paquetes");
  ...
  const submitEvent = async () => {
    try {
      ...
      setShipLoading(true);
      const payload = {
        shipmentTracking: shipmentTracking.trim(),
        status,
        note: note.trim(),
  ...
  const downloadAllLabelsZip = async () => {
    if (!shipmentTracking.trim())
      return toast.error("Indica tracking de envío maestro");
    try {
      const url = `/admin/shipments/${encodeURIComponent(
        shipmentTracking.trim()
      )}/labels.zip`;
      ...
      {tab === "packages" && (
        <div className="rounded-2xl border border-neutral-200/70 dark:border-neutral-800 p-6 bg-white dark:bg-neutral-900">
          <div className="grid md:grid-cols-[1fr_240px_auto] gap-3">
            <input
              className={`${input} md:max-w-2xl`}
              placeholder="Tracking de paquete…"
              value={pkQ}
              onChange={(e) => setPkQ(e.target.value)}
            />
            <select
              className={input}
              value={pkStatus}
              onChange={(e) => setPkStatus(e.target.value)}
            >
          ...
          <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {pkList.map((p) => (
              <div
                key={p.id}
                className="rounded-xl border border-neutral-200/70 dark:border-neutral-800 p-4"
              >
                <div className="flex items-center justify-between">
                ...
                {/* ⬇ detalle con salto de línea habilitado */}
                <div className="text-xs text-neutral-600 dark:text-neutral-300 mt-2 space-y-0.5 break-words">
                  <div>
                    <span className="font-medium">Tracking maestro:</span>{" "}
                    {p.shipmentTracking || "—"}
                  </div>
                  <div>
                  ...
                  {/* ⬇ quitar "truncate" y forzar salto */}
                  <div className="whitespace-normal break-words">
                    <span className="font-medium">Destino:</span>{" "}
                    {p.recipientAddress || "—"}
                    {p.destCity || p.destState || p.destCountry
                      ? ` — ${[p.destCity, p.destState, p.destCountry]
                          .filter(Boolean)
                          .join(", ")}`
                      : ""}
                  ...
                  <div>
                    <span className="font-medium">Peso / Tamaño:</span>{" "}
                    {Number(p.pesoKg || 0).toFixed(2)} kg —{" "}
                    {p.dimensiones?.largoCm}×{p.dimensiones?.anchoCm}×
                    {p.dimensiones?.altoCm} cm
                  </div>
                </div>
  ...
  return (
    <tr className="border-t border-neutral-200/60 dark:border-neutral-800">
      <td className="py-2 pr-2">
        <input
          className="w-full bg-transparent border-b border-transparent focus:border-green-600 outline-none"
          value={row.nombre}
          onChange={(e) => setRow((r) => ({ ...r, nombre: e.target.value }))}
        />
      </td>
      <td className="py-2 pr-2">
        <input
      ...
      <td className="py-2 pr-2">
        <select
          ...
          value={row.rol}
          onChange={(e) => setRow((r) => ({ ...r, rol: e.target.value }))}
        >
      ...
      <td className="py-2 pr-2">
        <input
          ...
          className="h-4 w-4 accent-green-700"
          checked={!!row.activo}
          onChange={(e) => setRow((r) => ({ ...r, activo: e.target.checked }))}
        />
      </td>
      <td className="py-2">
        <button
